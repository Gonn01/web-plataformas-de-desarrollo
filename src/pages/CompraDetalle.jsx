  ...
  // Datos mock para mostrar progreso, etc.
    ...
    <div className="min-h-dvh w-full bg-background-dark text-white font-display">
        ...
        <div className="flex flex-wrap gap-2 text-sm">
            <a className="text-[#9eb7a8] hover:text-primary transition-colors" href="#">Inicio</a>
            <span className="text-[#9eb7a8]">/</span>
            <a className="text-[#9eb7a8] hover:text-primary transition-colors" href="#">Debo</a>
            <span className="text-[#9eb7a8]">/</span>
        ...
        <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
            <div>
            <p className="text-4xl font-black tracking-[-0.033em]">Compra Notebook</p>
            <div className="mt-2">
          ...
          <section className="mt-8 pt-6 border-t border-gray-800">
            ...
            <div className="mt-4 p-4 rounded-lg bg-red-500/10 border border-red-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-gray-400">Una vez eliminada, esta acción no se puede deshacer.</p>
              </div>
...
function Cuota({ icon, title, venc, monto, paid, next }) {
  const base =
    "flex items-center justify-between p-4 rounded-lg shadow-sm bg-[#111714]";
  const mods = paid
    ? "opacity-60"
    : next
  ...
  return (
    <div className={`${base} ${mods}`}>
      <div className="flex items-center gap-4">
        <span className={`material-symbols-outlined ${paid ? "text-primary" : next ? "text-primary animate-pulse" : "text-gray-600"}`}>
          {icon}
        </span>
        <div className="flex flex-col">
          <p className={`font-medium ${paid ? "line-through text-white" : "text-white"}`}>{title}</p>
          <p className="text-sm text-[#9eb7a8]">Vencimiento: {venc}</p>
        </div>
      </div>
      <p className={`font-semibold ${paid ? "line-through text-white" : "text-white"}`}>{monto}</p>
    </div>
  );
}
